...
public class CustomTokenService
{
    ...
    public string GenerateToken(string claim, int userId)
    {
        DeleteExpiredTokens();
        var token = GenerateRandomToken();
        _tokens[token] = new TokenData(claim, userId, DateTime.UtcNow.AddMinutes(5));
        return token;
    }
    ...
    public TokenData? ValidateToken(string token, string claim)
    {
        ...
        if (_tokens.TryGetValue(token, out var tokenData))
        {
            ...
            {
                return tokenData; // Token ist gültig
            }
        }
